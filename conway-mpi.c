...
static void compute_cpu(bool *in, bool *out, int s) {
  ...
  for (int y = 1; y <= s; ++y) {
    for (int x = 1; x <= s; ++x) {
      int alive = in[(y - 1) * stride + (x - 1)] + in[(y - 1) * stride + x] +
                  in[(y - 1) * stride + (x + 1)] + in[y * stride + (x - 1)] +
                  in[y * stride + (x + 1)] + in[(y + 1) * stride + (x - 1)] +
                  in[(y + 1) * stride + x] + in[(y + 1) * stride + (x + 1)];
      bool cur = in[y * stride + x];
      out[y * stride + x] =
          (cur && (alive == 2 || alive == 3)) || (!cur && alive == 3);
    }
  }
}
...
int main(int argc, char **argv) {
  ...
  if (argc != 5) {
    fprintf(
        stderr,
        "USAGE: %s <InputFilePath> <OutputDirectory> <Steps> <WriteSteps>\n",
        argv[0]);
    MPI_Finalize();
    return 1;
  }
  ...
  MPI_Request reqs[8];
  MPI_Status status[8];
  rc = MPI_File_open(MPI_COMM_WORLD, inputFile, MPI_MODE_RDONLY, MPI_INFO_NULL,
                     &inFile);
  ...
  MPI_Type_vector(s, 1, s + 2, MPI_C_BOOL, &column_type);
  ...
  for (int k = 1; k <= s; ++k) {
    ...
    MPI_File_read_at_all(inFile, offset + ((i + (w * (k - 1))) * sizeof(bool)),
                         start, len, MPI_C_BOOL, MPI_STATUS_IGNORE);
  }
  ...
  for (int j = 1; j <= numSteps; ++j) {
    ...
    t0 = getticks();
    MPI_Isend(local + (s + 2) + s, 1, column_type, right_rank, HALO_MSG,
              MPI_COMM_WORLD, reqs + 0);
    MPI_Irecv(local + (s + 2) + (s + 1), 1, column_type, right_rank, HALO_MSG,
              MPI_COMM_WORLD, reqs + 1);

    MPI_Isend(local + (s + 2) + 1, 1, column_type, left_rank, HALO_MSG,
              MPI_COMM_WORLD, reqs + 2);
    MPI_Irecv(local + (s + 2) + 0, 1, column_type, left_rank, HALO_MSG,
              MPI_COMM_WORLD, reqs + 3);

    MPI_Waitall(4, reqs, status);

    MPI_Isend(local + (s * (s + 2)), s + 2, MPI_C_BOOL, bottom_rank, HALO_MSG,
              MPI_COMM_WORLD, reqs + 0);
    MPI_Irecv(local, s + 2, MPI_C_BOOL, top_rank, HALO_MSG, MPI_COMM_WORLD,
              reqs + 1);

    MPI_Isend(local + s + 2, s + 2, MPI_C_BOOL, top_rank, HALO_MSG,
              MPI_COMM_WORLD, reqs + 2);
    MPI_Irecv(local + ((s + 1) * (s + 2)), s + 2, MPI_C_BOOL, bottom_rank,
              HALO_MSG, MPI_COMM_WORLD, reqs + 3);
    ...
    if (writeSteps && j < numSteps) {
      ...
      for (int k = 1; k <= s; ++k) {
        ...
        MPI_Offset byte_offset = offset + ((process_grid_row * w) +
                                           process_grid_col + (w * (k - 1))) *
                                              sizeof(bool);
        MPI_File_write_at_all(outFile, byte_offset, start, s, MPI_C_BOOL,
                              MPI_STATUS_IGNORE);
      }
  ...
  // Write the final step outside the timed window (always, for validation).
  {
    ...
    for (int k = 1; k <= s; ++k) {
      ...
      MPI_Offset byte_offset =
          offset + ((process_grid_row * w) + process_grid_col + (w * (k - 1))) *
                       sizeof(bool);
      MPI_File_write_at_all(outFile, byte_offset, start, s, MPI_C_BOOL,
                            MPI_STATUS_IGNORE);
    }
  ...
  printf("Rank %2d | Total: %8.4f s | Comm: %8.4f s (%5.1f%%) | Compute: %8.4f "
         "s (%5.1f%%) | IO: %8.4f s (%5.1f%%)\n",
         world_rank, total_time, comm_time, 100.0 * comm_time / total_time,
         compute_time, 100.0 * compute_time / total_time, io_time,
         100.0 * io_time / total_time);
